# Tutorial Completo: Deploy do Smart Contract no Remix e Configuração da DApp

## 📋 Sumário

1. [Pré-requisitos](#pré-requisitos)
2. [Criando o Contrato no Remix](#criando-o-contrato-no-remix)
3. [Compilando o Contrato](#compilando-o-contrato)
4. [Fazendo o Deploy na Testnet Sepolia](#fazendo-o-deploy-na-testnet-sepolia)
5. [Obtendo o Endereço do Contrato](#obtendo-o-endereço-do-contrato)
6. [Configurando o Arquivo .env](#configurando-o-arquivo-env)
7. [Configurando a Aplicação Web](#configurando-a-aplicação-web)
...
- Crie uma carteira ou faça login na sua carteira existente
- **Anote sua frase seed em local seguro!**

### 2. ETH de Testnet Sepolia
- Acesse um faucet para obter ETH de teste:
  - [Sepolia Faucet](https://sepoliafaucet.com/)
  - [Alchemy Sepolia Faucet](https://sepoliafaucet.alchemy.com/)
  - [QuickNode Sepolia Faucet](https://faucet.quicknode.com/ethereum/sepolia)
- Você precisará de pelo menos 0.01 ETH para o deploy

### 3. Acesso ao Remix IDE
- Acesse [Remix IDE](https://remix.ethereum.org/)
- Não é necessário instalação, é uma ferramenta online

---

## 🛠️ Criando o Contrato no Remix
...
1. No painel `File Explorers` (à esquerda), clique em `Create New File`
2. Nomeie o arquivo como: `RegistroDeUsuariosComRecompensa.sol`

### 3. Copiar o Código do Contrato
Copie o código completo do arquivo `src/contracts/RegistroDeUsuariosComRecompensa.sol` do projeto:
...
### 1. Selecionar Compilador
1. No painel `Solidity Compiler` (ícone de compilador no lado esquerdo)
2. Selecione a versão: `0.8.20` ou superior
3. Clique no botão azul **Compile RegistroDeUsuariosComRecompensa.sol**

### 2. Verificar Compilação
- ✅ Se a compilação for bem-sucedida, você verá uma marca verde
- ❌ Se houver erros, verifique a versão do compilador e o código

### 3. ABI
- Após compilar, a ABI será gerada automaticamente
- Você pode visualizála na aba `ABI` do compilador
- Esta ABI será usada pela aplicação web

---

## 🚀 Fazendo o Deploy na Testnet Sepolia

### 1. Conectar MetaMask ao Remix
1. No painel `Deploy & Run Transactions` (ícone de seta para baixo)
2. Em `ENVIRONMENT`, selecione `Injected Provider - MetaMask`
3. O MetaMask abrirá automaticamente pedindo permissão
4. Clique em `Conectar` e depois em `Permitir`

### 2. Verificar Rede
1. Verifique se o MetaMask está na rede **Sepolia**
2. Se não estiver, mude para Sepolia no MetaMask
3. Confirme que você tem saldo de ETH de teste

### 3. Configurar Deploy
1. Em `CONTRACT`, selecione `RegistroDeUsuariosComRecompensa`
2. Em `DEPLOY`, clique no botão laranja **Deploy**

### 4. Confirmar Transação
1. O MetaMask abrirá uma janela de confirmação
2. Verifique os detalhes:
   - **Gas Limit**: Geralmente entre 2.000.000 - 3.000.000
   - **Gas Price**: Varia conforme a rede
   - **Custo Total**: Geralmente 0.001 - 0.005 ETH
3. Clique em **Confirmar**

### 5. Aguardar Deploy
- Aguarde a confirmação da transação (geralmente 15-30 segundos)
- Você verá o status no painel inferior do Remix
...
### 1. Copiar Endereço
1. Após o deploy bem-sucedido, o endereço aparecerá em:
   - Painel `Deployed Contracts` na parte inferior do Remix
   - Terminal do Remix
   - Histórico de transações do MetaMask

2. O endereço terá o formato: `0x1234567890abcdef1234567890abcdef12345678`

3. **Copie este endereço** - você precisará dele para configurar a aplicação

### 2. Verificar no Etherscan
1. Copie o endereço do contrato
2. Acesse [Sepolia Etherscan](https://sepolia.etherscan.io/)
3. Cole o endereço na busca
4. Você deverá ver:
   - **Contract**: ✓ (indicando que é um contrato válido)
   - **Creator**: Seu endereço do MetaMask
   - **Transactions**: A transação de deploy
...
Edite o arquivo `.env` e adicione o seguinte conteúdo:
...
# Chave Privada (APENAS PARA DESENVOLVIMENTO LOCAL)
# NÃO USE EM PRODUÇÃO! Use MetaMask para deploy em produção
PRIVATE_KEY=your_private_key_here

# Configurações da Aplicação
APP_NAME=Registro de Usuários com Recompensa
APP_VERSION=1.0.0
```

### 3. Substituir Valores
- **CONTRACT_ADDRESS**: Substitua pelo endereço do seu contrato
- **PRIVATE_KEY**: (Opcional) Apenas se for usar scripts de deploy local
- **RPC_URL**: (Opcional) Se tiver uma conta Infura

### 4. Adicionar ao .gitignore
Verifique se `.env` está no seu arquivo `.gitignore`:
...
### 1. Atualizar Configuração no JavaScript
No arquivo `web/js/app.js`, adicione o endereço do contrato:

```javascript
// No construtor da classe DApp, adicione:
this.contractAddress = '0x1234567890abcdef1234567890abcdef12345678'; // Substitua pelo seu endereço
```

### 2. Ou Usar Variável de Ambiente
Se preferir usar o arquivo .env, você pode modificar o `web/js/app.js` para ler do arquivo de configuração:

```javascript
// No início do arquivo web/js/app.js
...
### 3. Verificar ABI
A ABI já está incluída no arquivo `web/js/contract.js`, mas se precisar atualizar:

1. No Remix, após compilar, copie a ABI
2. Cole no arquivo `web/js/contract.js` na variável `contractABI`
...
4. Teste as funcionalidades:
   ...
   - ✅ Distribuir recompensas (apenas owner)

### 3. Verificar Logs
- Abra o console do navegador (F12)
- Verifique os logs para garantir que tudo está funcionando
- Os logs detalhados são salvos na pasta `logs/`

---

## 🔧 Solução de Problemas

### Problema 1: "Contrato não encontrado"
**Causa:** Endereço do contrato incorreto ou rede errada
**Solução:**
1. Verifique se o endereço está correto
2. Confirme que está na rede Sepolia
3. Verifique o contrato no Etherscan

### Problema 2: "MetaMask não encontrado"
**Causa:** MetaMask não instalado ou habilitado
**Solução:**
1. Instale o MetaMask
2. Recarregue a página
3. Verifique se o MetaMask está desbloqueado

### Problema 3: "Gas insuficiente"
**Causa:** Sem ETH de teste suficiente
**Solução:**
1. Obtenha mais ETH no faucet Sepolia
2. Aguarde a confirmação do depósito
3. Tente novamente

### Problema 4: "Usuário não é o owner"
**Causa:** Tentando distribuir recompensas sem ser o deployer
**Solução:**
1. Apenas a carteira que fez o deploy pode distribuir recompensas
2. Use a mesma carteira do deploy

### Problema 5: "Rede incorreta"
**Causa:** MetaMask em rede diferente da Sepolia
**Solução:**
1. Abra o MetaMask
2. Clique na rede atual
3. Selecione "Show test networks"
4. Escolha "Sepolia"
...
- [ ] Aplicação web atualizada com endereço
...
Você conseguiu:
✅ Criar e compilar um Smart Contract no Remix  
✅ Fazer o deploy na testnet Sepolia  
✅ Obter o endereço do contrato  
✅ Configurar a aplicação web  
✅ Testar todas as funcionalidades  

Seu Smart Contract está agora funcionando na blockchain Ethereum!
...
- [Documentação MetaMask](https://docs.metamask.io/)
